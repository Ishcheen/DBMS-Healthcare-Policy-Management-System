...
public class Extract extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int userKey=(int)request.getAttribute("userKey");
		int policyKey=(int)request.getAttribute("policyKey");
		int agentKey=(int)request.getAttribute("agentKey");
		System.out.println(userKey);
		System.out.println(policyKey);
		
		ArrayList<Policy> policyOutput=searchPolicy(policyKey);
	    request.setAttribute("policyData", policyOutput);
	    ArrayList<Agent> agentOutput=searchAgent(agentKey);
	    request.setAttribute("agentData", agentOutput);
	    ArrayList<Claimant> claimantOutput=searchClaimant(userKey);
	...
	public static ArrayList<Policy> searchPolicy(int value) {
		...
		try {
			...
			String sql="select * from policy where policy_id= '"+value+"'";
	...
	public static ArrayList<Agent> searchAgent(int value) {
		PostgreSqlJDBC obj_connection=new PostgreSqlJDBC();
		Connection connection=obj_connection.connect();
		PreparedStatement p= null;
		ResultSet rs=null;
		...
		try {
			...
			String sql="select * from agent where agent_id= '"+value+"'";
			p=connection.prepareStatement(sql);
			rs=p.executeQuery();
			while(rs.next()) {
